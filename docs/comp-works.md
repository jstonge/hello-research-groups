...
├─ comp # Computational
│  ├─ sims       # simulations
│  │  ├─ numerical   # mcmc, gibbs sampling, odes, ...
│  │  └─ abms   # agent-based models
│  └─ empirical   # there is real data analysis going on. This is not just about demonstrating a method.
├─ conceptual # only words and ideas; includes review, editorial piece, 'manifestos', etc...
└─ theory  
   └─ math  # math paper without any code
```
...
The idea that a paper is computational or not is very coarse grained. Some papers are computational by virtue of running some simulations as it is often the case in physics, while computational social science today can combine data source such as web scrapping and APIs, simulate a generative proces, fit a model, validate, and finally build tailored data visulizations. Obviously, the latter case is more 'computational' in that it required more code; the code for the data pipeline, simulation, analysis, and visualization. It is not all that different than the notion of technological complexity, where more bits of innovation in tools is a measure of complexity in cultural evolution. Analogous to that, we could count the bits of computation in a paper. It is more cumbersome, as there are more things to keep track by paper:

- Is there data. If so, where is it coming from? API? Web Scraping? Survey data?
- Did the authors simulated their data before fitting it?
- Are the data visualization more fancy than the default?

One dumb way to get that would be to match for key words, then disambiguate their meaning. For instance, you look for occurences of _simulation_ in a paper, then you have a model that classify the intent behind its mention. Was it because the authors _run_  simulations to gain insights from their problem? Do they critic someone else using simulation? Perhap they just _mention_ the idea of simulation en passant. Similarly for few other key words: _agent-based modeling_ [sims], _topic_modeling_[empirical], _data_[empirical], _github_[F/OSS], corpora[empirical], computational ______, any programming language, any hardware, etc.
